<x-layouts.base>
        ...
        <form wire:submit="check" method="post" class="w-full lg:px-52 h-fit px-5">
            <div class="lg:flex lg:flex-row shadow-md">
                <input type="text" required class="p-3 w-full text-black bg-white placeholder:text-gray-400"
                    wire:model="tiket" name="tiket" value="{{ old('tiket') }}"
                    placeholder="contoh : KTK-0507253-UNGASAN">
                <button type="submit"
            ...
            <div class="w-full lg:mt-16 mt-5 text-center  text-white">
                ...
                <h1 class="font-bold lg:text-5xl text-3xl lg:mt-10 mt-5">{{ $laporans['total'] }} Laporan</h1>

                <div class="cont grid lg:grid-cols-6 grid-cols-4 gap-5 lg:mt-10 mt-5">
                    @foreach ($laporans['stats'] as $item)
                        ...
                        <div class="fitur lg:w-50 flex flex-col items-center">
                            <div class="desc mt-2">
                                <h5 class="text-center lg:text-5xl text-3xl font-bold mb-5">{{ $item['total'] }}</h5>
                                <p class="text-center lg:text-lg text-white capitalize">{{ $name }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    <section id="progress" class="flex flex-row justify-center">
        <div class="progress-grp relative p-20">
            <div class="cont grid lg:grid-cols-5 gap-5 relative">
                <h5 class="font-bold lg:text-2xl text-center text-gray-400 mb-5 col-span-5">PROSES PENGAJUAN LAPORAN
                </h5>
